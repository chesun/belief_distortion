...
def draw(canvas_dim, matrix_dim, p, save_path, file_name_shell, top_half): 
    ...
    def save_graph():
        ...
        return png_complete_name,  file_name+'.png'
    ...
    if top_half == True:
        ...
        # complete file name including path, and just file name for half matrix
        half_complete_name, half_file_name = save_graph()
        ...
        # complete file name including path, and just file name for complete matrix 
        whole_complete_name, whole_file_name = save_graph()
    ...
    detail_dict = {"half_position": "top" if draw_top == True else "bottom", "half red": red_half, "total red": red_total, "half_complete_name": half_complete_name, "whole_complete_name": whole_complete_name, "half_file_name": half_file_name, "whole_file_name": whole_file_name}
...
def draw_n_pairs(canvas_dim, matrix_dim, n, p):
    ...
    # write html img tag embed code to csv file 
    with open(os.path.join(save_path, f"mat_embed_html_tags.csv"), "w", newline="") as embed_code_csv:
        # img tags to generate in each row: half 1, half 2, whole 1, whole 2
        # html tag pre and suffixes
        tag_prefix = f"<img src=\"http://christinasun.net/asset_hosting_service/matrix_graphs/dim_{matrix_dim}_pairs_{n}/"
        tag_suffix = f"\" width=\"{canvas_dim}\" height=\"{canvas_dim}\" >"
        # initialize the dictionary which will be used to write to csv
        overall_dict_to_write = {}
        for pairindex in range(n):
            # in each row, there are 2 matrices, which equals 4 tags
            # dictionary for current pair filenames and details: keys include -> pair number, tag1, tag2, tag3, tag4, mat 1 half red, mat 2 half red, mat 1 total red, mat 2 total red
            pair_file_names = {}
            for matindex in range(2): # assign file names for half and whole matrices in the pair
                pairnum = pairindex + 1
                matnum = matindex + 1
                pair_file_names["pairnum"] = pairnum
                pair_file_names["halfpos"] = nred_overall[2 * pairindex]["half_position"]
                for type in ["half", "whole"]:
                    # html tag
                    pair_file_names[f"mat_{matnum}_{type}_tag"] = tag_prefix + nred_overall[2 * pairindex + matindex][f"{type}_file_name"] + tag_suffix
                # number of red
                for rednumtype in ["half", "total"]:
                    pair_file_names[f"mat_{matnum}_{rednumtype}_red"] = nred_overall[2 * pairindex + matindex][f"{rednumtype} red"]
            tag_fieldnames = list(pair_file_names.keys())
            tagwriter = csv.DictWriter(embed_code_csv, fieldnames=tag_fieldnames)
            if pairindex == 0:
                tagwriter.writeheader()
            tagwriter.writerow(pair_file_names)



    # this is a list of dictionaries, one dict for each matrix
    return nred_overall
...
# overall details: a list of matrix (half and complete) detail dictionaries
dim_10_overall_dict = draw_n_pairs(canvas_dim= 300, matrix_dim= 10, n = 20, p = 0.5)

dim_20_overall_dict = draw_n_pairs(canvas_dim= 400, matrix_dim= 20, n = 10, p = 0.5)
