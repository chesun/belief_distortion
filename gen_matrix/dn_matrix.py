...
class DotMatrix:
    ...
    def __init__(self, n_red, matrix_dim=10, canvas_dim=300):
        ...
        self.pad = canvas_dim / \
            (2 * matrix_dim) if matrix_dim <= 10 else canvas_dim/matrix_dim
        # space between dots
        ...
        self.mat_array = np.reshape(
            self.mat_array, newshape=(self.matrix_dim, self.matrix_dim))
    ...
    def setup(self):
        """ an internal method to setup a canvas and pen"""
    ...
    def draw_border(self):
        ...
        self.pen.penup()
        self.pen.goto(self.starting_x - self.pad,
                      self.starting_y + self.pad)  # top left corner
        self.pen.pendown()

        self.pen.goto(self.starting_x - self.pad, -
                      self.starting_y - self.pad)  # bottom left corner
        self.pen.goto(- self.starting_x + self.pad, -
                      self.starting_y - self.pad)  # bottom right corner
        self.pen.goto(- self.starting_x + self.pad,
                      self.starting_y + self.pad)  # top right corner
        # back to top left corner
        self.pen.goto(self.starting_x - self.pad, self.starting_y + self.pad)
