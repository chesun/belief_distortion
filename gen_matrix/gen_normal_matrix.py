...
def draw(sample, mat_dim, folder_name, folder_path, url_dir, img_size):
    ...
    for pair in sample:
        ...
        for n_red_total in pair:
            ...
            # save graph
            this_mat.save(filename=filename_half,
            ...
            # save graph
            this_mat.save(filename=filename_complete,
                          folder_name=folder_name, folder_path=folder_path)
...
def export_control_treat_data(treat_dicts, folder_path, folder_name):
    ...
    path = os.path.join(folder_path, folder_name,
                        f"{folder_name}"+"control_treat_loop_fields.csv")
...
def gen_matrix_pairs(mat_dim, n_pairs, set_num):
    ...
    # draw random sample
    sample_array = draw_sample(
        n_pairs=n_pairs, dist=dist, folder_name=paths.folder_name, folder_path=paths.folder_path)
...
def main():
    ...
    for i in range(constants.n_sets):
        gen_matrix_pairs(mat_dim=constants.mat_dim,
                         n_pairs=constants.n_pairs, set_num=i+1)
